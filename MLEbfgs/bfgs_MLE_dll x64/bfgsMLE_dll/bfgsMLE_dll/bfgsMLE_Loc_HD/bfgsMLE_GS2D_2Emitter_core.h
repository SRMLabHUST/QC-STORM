...
#define FittedFluoNum_2D_2E				2 // constant, don't modify

#define IterateNum_2D_2E				13  // total iteration number, fixed iteration number
#define IterateNum_2D_2E_bs				11 // bisection iteration to find best walk length


// number of fitting parameter
#define FitParaNum_2D_2E				8

#define FreeParaPerFluo					3

// fitting parameters ID
#define Fit2D_2E_Peak0					0
#define Fit2D_2E_XPos0					1
#define Fit2D_2E_YPos0					2

#define Fit2D_2E_Peak1					3
#define Fit2D_2E_XPos1					4
#define Fit2D_2E_YPos1					5

#define Fit2D_2E_Sigm					6
#define Fit2D_2E_Bakg					7




// better to enable
#define WLE_ENABLE_MultiEmitterFit_2D				(1 && WLE_ENABLE)
...
__device__ void MLELocalization_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float d0[][ThreadsPerBlock], float D0[], float* WLE_Weight, int tid);


template <int ROISize, int ROIPixelNum, int FitParaNum>
__device__ void NumericalGradient_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float* WLE_Weight, int tid);


template <int ROISize, int ROIPixelNum, int FitParaNum>
__device__ float MLEFit_LossFunction_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[], float* WLE_Weight, int tid);


template <int ROISize, int ROIPixelNum, int FitParaNum>
__device__ void PreEstimation_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[][ThreadsPerBlock], int tid);



template <int ROISize, int FitParaNum>
__global__ void bfgsMLELoc_Gauss2D_2E(float *d_LocArry, unsigned short *d_ImageROI, float *d_WLEPara, int MultiFitFluoNum_2E, FitPosInf_TypeDef* d_FitPosInf, CoreFittingPara *d_FitPara)
{
...
#define XYPosVaryBound		1.5f
	...
	int * d_MultiFitFluoNum_3E = d_FitPosInf->d_MultiFitFluoNum_3E;
	...
	for (int fcnt = 0; fcnt < ThreadsPerBlock; fcnt++)
	{
		...
		CurId = min(CurId, MultiFitFluoNum_2E - 1);

		int CurMapID = d_MultiFitFluoPos_2E[CurId];
...
#pragma unroll
	...
	float Distance[FittedFluoNum_2D_2E];
	...
	if (gid < MultiFitFluoNum_2E)
	{
		...
		// pre-estimation
		PreEstimation_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, Ininf, tid);
		...
		MLELocalization_GS2D_2E<ROISize, ROIPixelNum, FitParaNum_2D_2E, IterateNum_2D_2E, IterateNum_2D_2E_bs>(ImageROI, Ininf, grad, d0, &D0[tid][0], WLE_Weight, tid);


		// remove scaling factor
		Ininf[Fit2D_2E_Peak0][tid] = Ininf[Fit2D_2E_Peak0][tid] * AScalFactor / QE; // PeakPhoton
		Ininf[Fit2D_2E_Peak1][tid] = Ininf[Fit2D_2E_Peak1][tid] * AScalFactor / QE; // PeakPhoton

		float Background = Ininf[Fit2D_2E_Bakg][tid] * BScalFactor / QE; // background
		
		float SimgaX = sqrtf(0.5f / (Ininf[Fit2D_2E_Sigm][tid] * SScalFactor)); // PSF sigma
		...
		// molecule selection
		for (int cnt = 0; cnt <FittedFluoNum_2D_2E; cnt++)
		{
		...
		else
		{
			...
			// mark position for 3 emitter fitting
			int StorePos = atomicAdd(d_MultiFitFluoNum_3E, 1);

			if (StorePos < MaxPointNum)
			{
				d_MultiFitFluoPos_3E[StorePos] = CurFluoID;
			}
...
__device__ void MLELocalization_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float d0[][ThreadsPerBlock], float D0[], float* WLE_Weight, int tid)
{
	...
	// initialize
	NumericalGradient_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, Ininf, grad, WLE_Weight, tid);
...
#pragma unroll
		...
		ddat[0] = MLEFit_LossFunction_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, &xd[0], WLE_Weight, tid);
		ddat[1] = MLEFit_LossFunction_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, &xd[FitParaNum], WLE_Weight, tid);
		...
		for (int cnt = 0; cnt < IterateNum_bs; cnt++)
		{
			...
			if (cnt < IterateNum_bs - 1)
			{
				...
				ddat[xdsel] = MLEFit_LossFunction_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, &xd[xdsel*FitParaNum], WLE_Weight, tid);
			}
		}
...
#pragma unroll
	...
	for (cnt = 0; cnt<FitParaNum; cnt++)
	{
		...
		tgradn = MLEFit_LossFunction_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, tIninf, WLE_Weight, tid);

		tIninf[cnt] = tIninf[cnt] + 0.004f;
		tgradp = MLEFit_LossFunction_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, tIninf, WLE_Weight, tid);
...
#define Peak0_Scaled   Ininf[Fit2D_2E_Peak0] * AScalFactor
#define XPos0_Scaled   Ininf[Fit2D_2E_XPos0]
#define YPos0_Scaled   Ininf[Fit2D_2E_YPos0]

#define Peak1_Scaled   Ininf[Fit2D_2E_Peak1] * AScalFactor
#define XPos1_Scaled   Ininf[Fit2D_2E_XPos1]
#define YPos1_Scaled   Ininf[Fit2D_2E_YPos1]

#define Sigm_Scaled   Ininf[Fit2D_2E_Sigm] * SScalFactor
#define Bakg_Scaled   Ininf[Fit2D_2E_Bakg] * BScalFactor
...
#pragma unroll
	...
	Ininf[Fit2D_2E_Peak0][tid] = 0.8f * Amp * rAScalFactor;
	Ininf[Fit2D_2E_XPos0][tid] = ROICenter;
	Ininf[Fit2D_2E_YPos0][tid] = ROICenter;

	Ininf[Fit2D_2E_Peak1][tid] = 0.8f * Amp * rAScalFactor;
	Ininf[Fit2D_2E_XPos1][tid] = XPos1_Ini;
	Ininf[Fit2D_2E_YPos1][tid] = YPos1_Ini;


	Ininf[Fit2D_2E_Sigm][tid] = SigmaX1 * rSScalFactor;

	Ininf[Fit2D_2E_Bakg][tid] = 0.8f * Bkg * rBScalFactor;

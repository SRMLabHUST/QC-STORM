...
__device__ void poissonfGradient_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float* WLE_Weight, int tid);


template <int ROISize, int ROIPixelNum, int FitParaNum>
__device__ float MLEFit_TargetF_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[], float* WLE_Weight, int tid);
...
__device__ void MLELocalization_GS2D_2E(float ImageROI[][ROIPixelNum], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float d0[][ThreadsPerBlock], float D0[], float* WLE_Weight, int tid)
{
	...
	// initialize
	poissonfGradient_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, Ininf, grad, WLE_Weight, tid);
...
#pragma unroll
		...
		ddat[0] = MLEFit_TargetF_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, &xd[0], WLE_Weight, tid);
		ddat[1] = MLEFit_TargetF_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, &xd[FitParaNum], WLE_Weight, tid);
		...
		for (int cnt = 0; cnt < IterateNum_bs; cnt++)
		{
			...
			if (cnt < IterateNum_bs - 1)
			{
				...
				ddat[xdsel] = MLEFit_TargetF_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, &xd[xdsel*FitParaNum], WLE_Weight, tid);
			}
		}
...
#pragma unroll
	...
	for (cnt = 0; cnt<FitParaNum; cnt++)
	{
		...
		tgradn = MLEFit_TargetF_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, tIninf, WLE_Weight, tid);

		tIninf[cnt] = tIninf[cnt] + 0.004f;
		tgradp = MLEFit_TargetF_GS2D_2E<ROISize, ROIPixelNum, FitParaNum>(ImageROI, tIninf, WLE_Weight, tid);

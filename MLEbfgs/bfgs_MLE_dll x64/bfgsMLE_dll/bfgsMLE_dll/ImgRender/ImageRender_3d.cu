...
__global__ void FluoRenderWithPrec_3D(float *d_LocArry, float *d_SRIntensityImg, float *d_SRColorMapImg, int *d_MaxImageVal, int *d_HistMaxDat, int RenderingMode, float FixedlocPrec, float SNR_th, float PixelSize, float PixelZoom, float MinZDepth, float MaxZDepth, int SRImageWidth, int SRImageHigh, int FluoNum)
{
	//    int tid = threadIdx.x;
	...
	float ZDepthTh = 5 * max(abs(MinZDepth), abs(MaxZDepth));
	...
	if (gid<FluoNum)
	{
		...
		// valid fluos
		if ((CurSNR >= SNR_th) && (PeakPhoton > 1.0f) && (abs(RendZPos) < ZDepthTh) && (RendXPos > RendROIWidth) && (RendXPos < SRImageWidth - 1 - RendROIWidth) && (RendYPos > RendROIWidth) && (RendYPos < SRImageHigh - 1 - RendROIWidth))
		{
...
__global__ void RenderSaveImg_3D(float *d_SRIntensityImg, float *d_SRColorMapImg, char* d_RendImg, float MaxIntensity, float MinZDepth, float MaxZDepth, int ColorMode_3D, int RGBImageEncodeMode, int SRImageWidth, int SRImageHigh)
{
	//    int tid = threadIdx.x;
	int gid = threadIdx.x + blockDim.x*blockIdx.x;
	...
	if (curx < SRImageWidth)
	{
		...
		for (int cury = 0; cury< SRImageHigh; cury++)
		{
			...
			float CurZDepth = d_SRColorMapImg[FillImgOffset] / d_SRIntensityImg[FillImgOffset]; // in nm
...
void ImgRender_GetSaveImg_3D(ImageRenderData_TypeDef *h_RendData, LocalizationPara & LocPara, float BrightRatio, int RGBImageEncodeMode, cudaStream_t cstream)
{
	...
	cudaMemcpyAsync(h_RendData->h_MaxImageVal, h_RendData->d_MaxImageVal, sizeof(int), cudaMemcpyDeviceToHost, cstream);

	float MaxIntensity = LightRatio*(*h_RendData->h_MaxImageVal);
	*/

	float MaxHistVal = 12000.0f;

	float MaxIntensity = GetSRImageHistgram(h_RendData->d_SRIntensityImg, h_RendData->d_SRImageHist, h_RendData->h_SRImageHist, MaxHistVal, LocPara.SRImageWidth, LocPara.SRImageHigh, cstream);
	MaxIntensity = MaxIntensity*BrightRatio;

	int BlockDim = ThreadsPerBlock;
	int BlockNum = (LocPara.SRImageWidth + ThreadsPerBlock - 1) / ThreadsPerBlock;

	RenderSaveImg_3D << <BlockNum, BlockDim, 0, cstream >> >(h_RendData->d_SRIntensityImg, h_RendData->d_SRColorMapImg, h_RendData->d_SaveRendImg, MaxIntensity, LocPara.MinZDepth, LocPara.MaxZDepth, LocPara.ColorMode_3D, RGBImageEncodeMode, LocPara.SRImageWidth, LocPara.SRImageHigh);


	cudaMemcpyAsync(h_RendData->h_SaveRendImg, h_RendData->d_SaveRendImg, LocPara.SRImageWidth*LocPara.SRImageHigh * 4 * sizeof(char), cudaMemcpyDeviceToHost, cstream);
	cudaStreamSynchronize(cstream);
...
__global__ void gpuGetSRImageHistgram(float *d_SRIntensityImg, float *d_SRImageHist, float HistGap, int SRImageWidth, int SRImageHigh)
{
	int gid = threadIdx.x + blockDim.x*blockIdx.x;

	if (gid < SRImageWidth*SRImageHigh)
	{
		float CurData = d_SRIntensityImg[gid];

		int Position = CurData / HistGap - 1;

		if ((Position >= 0) && (Position < SRImageHist_DataLen))
		{
			atomicAdd(&d_SRImageHist[Position], 1);
		}
	}
...
{
	unsigned int TotalThreadNum = SRImageWidth*SRImageHigh;

	int BlockDim = ThreadsPerBlock;
	int BlockNum = ((TotalThreadNum + ThreadsPerBlock - 1) / ThreadsPerBlock);

	cudaMemsetAsync(d_SRImageHist, 0, SRImageHist_DataLen * sizeof(float), cstream);

	float HistGap = MaxHistVal / SRImageHist_DataLen;

	gpuGetSRImageHistgram << <BlockNum, BlockDim, 0, cstream >> > (d_SRIntensityImg, d_SRImageHist, HistGap, SRImageWidth,  SRImageHigh);

	cudaMemcpyAsync(h_SRImageHist, d_SRImageHist, SRImageHist_DataLen * sizeof(float), cudaMemcpyDeviceToHost, cstream);
	cudaStreamSynchronize(cstream);
	...
	float RenderGap = SRImageHist_DataLen - 1;

	for (int cnt = 0; cnt < SRImageHist_DataLen; cnt++)
	{
		TotalNum += h_SRImageHist[cnt];
	}
	for (int cnt = 0; cnt < SRImageHist_DataLen; cnt++)
	{
		CurNum += h_SRImageHist[cnt];
		CurRatio = CurNum / TotalNum;

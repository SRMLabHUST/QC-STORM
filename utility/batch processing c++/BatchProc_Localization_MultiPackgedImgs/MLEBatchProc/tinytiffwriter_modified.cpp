...
struct TinyTIFFFile {
    ...
    /* \brief file position (from _ftelli64) of the first byte of the previous IFD/frame header */
...
/*! \brief wrapper around _ftelli64
    \ingroup tinytiffwriter
    \internal
 */
...
/*! \brief wrapper around _fseeki64
    \ingroup tinytiffwriter
    \internal
 */
...
/*! \brief wrapper around _fseeki64(..., FILE_CURRENT)
    \ingroup tinytiffwriter
    \internal
 */
...
inline void TinyTIFFWriter_startIFD(TinyTIFFFile* tiff, int hsize=TIFF_HEADER_SIZE) {
    if (!tiff) return;
    tiff->lastStartPos=TinyTIFFWriter_ftell(tiff);//_ftelli64(tiff->file);
...
inline void TinyTIFFWriter_endIFD(TinyTIFFFile* tiff, int hsize=TIFF_HEADER_SIZE) {
    if (!tiff) return;
    //long startPos=_ftelli64(tiff->file);
...
TinyTIFFFile* TinyTIFFWriter_open(const char* filename, uint16_t bitsPerSample, uint32_t width, uint32_t height) {
    ...
    if (TinyTIFFWriter_fOK(tiff)) {
        ...
        tiff->lastIFDOffsetField=TinyTIFFWriter_ftell(tiff);//_ftelli64(tiff->file);
...
void TinyTIFFWriter_writeImage(TinyTIFFFile* tiff, void* data, int SamplesPerPixel) {
	if (!tiff) return;
	...
	TinyTIFFWriter_writeIFDEntrySHORT(tiff, TIFF_FIELD_SAMPLESPERPIXEL, SamplesPerPixel);
	...
	TinyTIFFWriter_fwrite(data, tiff->width*tiff->height*(tiff->bitspersample / 8), SamplesPerPixel, tiff);

...
__global__ void gpuFindConsecutiveROI(unsigned short * d_ROIMem, int ROISize, int *d_ForwardLinkID, int *d_BackwardLinkID, int FluoNum)
{
	...
	const int ROIDataSize = ROISize*(ROISize + 1);

	const int AddrOffset = ROIDataSize*gid + ROISize*ROISize;
	...
	if (gid < FluoNum)
	{
		...
		{
			int NextAddrOffset = ROIDataSize*cnt + ROISize*ROISize;
		...
		if (d_ForwardLinkID[gid] == 0)
		{
			if (ConsecutiveROINum > 0)
			{
				for (int i = 0; i < ROIDataSize; i++)
				{
					...
					while (NextValid)
					{
						int AddrOffset = ROIDataSize*CurId;

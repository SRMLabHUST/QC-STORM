...
void FluoStatisticData_TypeDef::CalcLocalizationDensity2D(float *h_LocArry, LocalizationPara & LocPara, int FluoNum, cudaStream_t cstream)
{
	...
	int FilterStartFrame = StartFrame + FrameNum * 2 / 10;
	int FilterEndFrame = StartFrame + FrameNum * 7 / 10;
	...
	for (CurFillerFrame = FilterStartFrame; CurFillerFrame < FilterEndFrame; CurFillerFrame++)
	{
		int oNeighbor0_Num, oTotalFluo;


		GetOverlapMolecules(&oNeighbor0_Num, &oTotalFluo, RadiusTh_pixel, CurFillerFrame, LocPara, FluoNum, cstream);

		Overlap1Num += oNeighbor0_Num;
		TotalNum += oTotalFluo;
	}
	...
	if (TotalNum > 100)
	{
		MeanLocDensity2D = GetActivationDensity((float)Overlap1Num / TotalNum, RadiusTh_um);
	}
	else
	{
		MeanLocDensity2D = 0;
...
void FluoStatisticData_TypeDef::GetOverlapMolecules(int*oNeighbor0_Num, int *oTotalFluo, float RadiusTh_pixel, int CurFrame, LocalizationPara & LocPara, int FluoNum, cudaStream_t cstream)
{

...
__device__ static void MatMulVector_3p(float D0[], float grad[][ThreadsPerBlock], float d0[][ThreadsPerBlock], int tid);
__device__ static void ConstructD0_3p(float D0[], float sk[], float yk[], int tid);
__device__ static void IninfConstrain_3p(float Ininf[][ThreadsPerBlock], int tid);
__device__ static void VectorAddMul1_3p(float oVector[], float Ininf[][ThreadsPerBlock], float d0[][ThreadsPerBlock], float coeff, int tid);

#define OutParaNum			12 // a,x,y,f,a,x,y,f,a,x,y,f

#define FitParaNum 			10
#define FittingNum			3
...
__device__ static void MLELocalization_3p(float subimg[][SharedImageWidth1], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float d0[][ThreadsPerBlock], float D0[], float PsfWidth, int tid);

__device__ static void PreLocolization_3p(float subimg[][SharedImageWidth1], float Ininf[][ThreadsPerBlock], int tid);

__device__ static void poissonfGradient_3p(float subimg[][SharedImageWidth1], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float PsfWidth, int tid);
__device__ static float poissonf_3p(float subimg[][SharedImageWidth1], float Ininf[], float PsfWidth, int tid);
...
#pragma unroll
	...
	PreLocolization_3p(ImageRegion,Ininf,tid);

	poissonfGradient_3p(ImageRegion, Ininf, grad, PsfWidth, tid);
...
#pragma unroll
	...
	MLELocalization_3p(ImageRegion, Ininf, grad, d0, &D0[tid][0], PsfWidth, tid);
...
//			ypos = 1024.0f - 3.0f + Ininf[2][tid];
	...
	}else
	{
		...
		if (Ininf[6][tid] > MaxAmp)MaxAmp = Ininf[6][tid];
		PointTh1 = (MaxAmp / Ininf[0][tid]) > 4;
		PointTh2 = (MaxAmp / Ininf[3][tid]) > 4;
		PointTh3 = (MaxAmp / Ininf[6][tid]) > 4;
		...
		if ((Ininf[7][tid]>1.5f) && (Ininf[7][tid]<7.5f) && (Ininf[8][tid]>1.5f) && (Ininf[8][tid] < 7.5f) && PointTh3)
		{
			Ininf[7][tid] = Ininf[7][tid] + ImageRegion[tid][SubRegionWidth + 0] - 4.0f; // X
			Ininf[8][tid] = Ininf[8][tid] + ImageRegion[tid][SubRegionWidth + 1] - 4.0f; // Y
		}
		else
		{
			Ininf[7][tid] = 0.0f; // X
		...
		d_LocArry[gid*OutParaNum +  3] = ImageRegion[tid][SubRegionWidth + 2];			// A

		d_LocArry[gid*OutParaNum + 4] = Ininf[3][tid] * 128.0f;			// X
		d_LocArry[gid*OutParaNum + 5] = Ininf[4][tid];			// Y
		d_LocArry[gid*OutParaNum + 6] = Ininf[5][tid];			// A
		d_LocArry[gid*OutParaNum +  7] = ImageRegion[tid][SubRegionWidth + 2];			// X

		d_LocArry[gid*OutParaNum + 8] = Ininf[6][tid] * 128.0f;			// Y
		d_LocArry[gid*OutParaNum + 9] = Ininf[7][tid];			// back
		d_LocArry[gid*OutParaNum + 10] = Ininf[8][tid];			// Y
		d_LocArry[gid*OutParaNum + 11] = ImageRegion[tid][SubRegionWidth + 2];			// back
...
__device__ static void MLELocalization_3p(float subimg[][SharedImageWidth1], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float d0[][ThreadsPerBlock], float D0[], float PsfWidth, int tid)
{
	...
	for (itcnt = 0; itcnt< IterateNum; itcnt++)
	{
		...
		td0_total = __fdividef( 15.0f, (td0[0] + td0[1] + td0[2] + td0[3] + td0[4] + td0[5] + td0[6] + td0[7] + td0[8] + td0[9])); // /10/1.5
		...
		d0[7][tid] = d0[7][tid] * td0_total;
		d0[8][tid] = d0[8][tid] * td0_total;
		d0[9][tid] = d0[9][tid] * td0_total;
		...
		VectorAddMul1_3p(&xd[0], Ininf, d0, 0.0001f, tid);
		VectorAddMul1_3p(&xd[FitParaNum], Ininf, d0, 1.0f, tid);

		ddat[0] = poissonf_3p(subimg, &xd[0], PsfWidth, tid);
		ddat[1] = poissonf_3p(subimg, &xd[FitParaNum], PsfWidth, tid);
		// 
		for (cnt = 0; cnt<IterateNum_bs; cnt++)
		{
			...
			{
				VectorAddMul1_3p(&xd[xdsel*FitParaNum], Ininf, d0, dpos[xdsel], tid);// xd=ininf+d0*scale
				ddat[xdsel] = poissonf_3p(subimg, &xd[xdsel*FitParaNum], PsfWidth, tid);
		...
		Ininf[7][tid] = Ininf[7][tid] + sk[7];
		Ininf[8][tid] = Ininf[8][tid] + sk[8];
		Ininf[9][tid] = Ininf[9][tid] + sk[9];


		IninfConstrain_3p(Ininf, tid); // 
		...
		if (itcnt<IterateNum - 1)
		{
			...
			poissonfGradient_3p(subimg, Ininf, grad, PsfWidth, tid);
			...
			yk[7] = grad[7][tid] - tgrad[7];
			yk[8] = grad[8][tid] - tgrad[8];
			yk[9] = grad[9][tid] - tgrad[9];

			ConstructD0_3p(D0, sk, yk, tid);
			MatMulVector_3p(D0, grad, d0, tid);
		}
	}
}
...
__device__ static float poissonf_3p(float subimg[][SharedImageWidth1], float Ininf[], float PsfWidth, int tid)
{
	...
	if (Ininf[7]<0.0f) Ininf[7] = 0.0f; // X
	if (Ininf[8]<0.0f) Ininf[8] = 0.0f; // Y
	if (Ininf[9]<0.0f) Ininf[9] = 0.0f; // bg
	...
	for (row = 0; row<RegionLen; row++)
	{
		for (col = 0; col<RegionLen; col++)
		{
			...
			IVal = 128.0f*Ininf[0] * __expf(-tdat) + Ininf[9] * 32.0f; // point 1


			tdat = ((colpos - Ininf[4])*(colpos - Ininf[4]) + (rowpos - Ininf[5])*(rowpos - Ininf[5]))*PsfWidth;
			IVal = IVal+ 128.0f*Ininf[3] * __expf(-tdat); // point 2

			tdat = ((colpos - Ininf[7])*(colpos - Ininf[7]) + (rowpos - Ininf[8])*(rowpos - Ininf[8]))*PsfWidth;
			IVal = IVal+ 128.0f*Ininf[6] * __expf(-tdat); // point 3
...
__device__ static void poissonfGradient_3p(float subimg[][SharedImageWidth1], float Ininf[][ThreadsPerBlock], float grad[][ThreadsPerBlock], float PsfWidth, int tid)
{
	...
	for (cnt = 0; cnt<FitParaNum; cnt++)
	{
		...
		tgradn = poissonf_3p(subimg, tIninf, PsfWidth, tid);

		tIninf[cnt] = tIninf[cnt] + 0.004f;
		tgradp = poissonf_3p(subimg, tIninf, PsfWidth, tid);
...
__device__ static void PreLocolization_3p(float subimg[][SharedImageWidth1], float Ininf[][ThreadsPerBlock], int tid)
{
	...
	curx = (((int)pSubImg[9][5]) / 256);
	cury = (((int)pSubImg[9][5]) % 256);

	Ininf[6][tid] = (pSubImg[cury][curx] - sum_bg)/128.0f; // /256.0f;	// Amplitude
	Ininf[7][tid] = curx + 0.5f; // sumDatx / sumdat - 0.5f;	// x
	Ininf[8][tid] = cury + 0.5f; // sumDaty / sumdat - 0.5f;	// y

	Ininf[9][tid] = sum_bg/32.0f; // /128.0f;	// bg
...
__device__ static void VectorAddMul1_3p(float oVector[], float Ininf[][ThreadsPerBlock], float d0[][ThreadsPerBlock], float coeff, int tid)
{
	...
	oVector[7] = d0[7][tid] * coeff + Ininf[7][tid];
	oVector[8] = d0[8][tid] * coeff + Ininf[8][tid];
	oVector[9] = d0[9][tid] * coeff + Ininf[9][tid];
...
__device__ static void IninfConstrain_3p(float Ininf[][ThreadsPerBlock], int tid)
{
	...
	if (Ininf[7][tid]<0.001f) Ininf[7][tid] = 0.001f; // bg
	if (Ininf[8][tid]<0.001f) Ininf[8][tid] = 0.001f; // sigma
	if (Ininf[9][tid]<0.001f) Ininf[9][tid] = 0.001f; // bg

}

__device__ static void ConstructD0_3p(float D0[], float sk[FitParaNum], float yk[FitParaNum], int tid)
{
	float divdat;
	...
	tdat10[7] = yk[7] * divdat;
	tdat10[8] = yk[8] * divdat;
	tdat10[9] = yk[9] * divdat;
	...
	tdat20[7] = sk[7] * divdat;
	tdat20[8] = sk[8] * divdat;
	tdat20[9] = sk[9] * divdat;
	...
	for (row = 0; row<FitParaNum; row++)
	{
		for (col = 0; col<FitParaNum; col++)
		{
			...
			ptD0[row][col] = pskyk[row][0] * pD0[0][col] + \
							 ...
							 pskyk[row][7] * pD0[7][col] + \
							 pskyk[row][8] * pD0[8][col] + \
							 pskyk[row][9] * pD0[9][col];		
		}
	}
	...
	for (row = 0; row<FitParaNum; row++)
	{
		for (col = 0; col<FitParaNum; col++)
		{
			...
			pD0[row][col] = ptD0[row][0] * pskyk[col][0] + \
							...
							ptD0[row][7] * pskyk[col][7] + \
							ptD0[row][8] * pskyk[col][8] + \
							ptD0[row][9] * pskyk[col][9];	
		}
	}
	// D0=D0+sksk;
	for (row = 0; row<FitParaNum; row++)
	{
		...
		pD0[row][7] = pD0[row][7] + psksk[row][7];
		pD0[row][8] = pD0[row][8] + psksk[row][8];
		pD0[row][9] = pD0[row][9] + psksk[row][9];
...
__device__ static void MatMulVector_3p(float D0[], float grad[][ThreadsPerBlock], float d0[][ThreadsPerBlock], int tid)
{
	...
	for (row = 0; row< FitParaNum; row++)
	{
		d0[row][tid] = -(pD0[row][0] * grad[0][tid] + 
						 ...
						 pD0[row][7] * grad[7][tid] + 
						 pD0[row][8] * grad[8][tid] + 
						 pD0[row][9] * grad[9][tid]);
	}
